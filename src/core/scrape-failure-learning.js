...
export const SCRAPE_FAILURE_LESSONS = [
  ...
  {
    key: 'captcha_or_checkpoint',
    category: 'login',
    match: /captcha|checkpoint|ยืนยันตัวตนของแพลตฟอร์ม/i,
    lesson: 'เจอ CAPTCHA/หน้ายืนยันตัวตน แล้วงานแดงถ้าเดาคลิกต่อ',
    prevention: 'หยุดบัญชีนั้นทันที ห้ามเดาคลิก แจ้งคนตรวจบัญชีแล้วเริ่มใหม่',
  },
  {
...
/** Expansion only: keep the original term, drop terms that already opened many and qualified none. */
export function filterZeroYieldExpansionTerms(positions, deadTerms, keepTerms = []) {
  const list = (Array.isArray(positions) ? positions : []).map((item) => String(item || '').trim()).filter(Boolean);
  if (!list.length) return list;
  const deadSet = new Set((deadTerms || []).map((term) => String(term || '').trim().toLocaleLowerCase('th-TH')).filter(Boolean));
  const keepSet = new Set((keepTerms || []).map((term) => String(term || '').trim().toLocaleLowerCase('th-TH')).filter(Boolean));
  if (!deadSet.size) return list;
  return list.filter((term) => {
    const key = term.toLocaleLowerCase('th-TH');
    return keepSet.has(key) || !deadSet.has(key);
  });
}
