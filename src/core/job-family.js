...
const DESC_TOOL = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      job_dna: { type: 'string', description: 'แก่นของงานที่ใช้ตัดสินว่าผู้สมัครยังอยู่ในสายงานเดียวกัน' },
      hard_filters: {
        type: 'array',
        description: 'เงื่อนไขบังคับที่ระบุชัดในเนื้องานเท่านั้น ห้ามอนุมานเพิ่ม',
        items: {
          type: 'object',
          properties: {
            field: { type: 'string', enum: ['license', 'skill', 'experience', 'education', 'location', 'other'] },
            value: { type: 'string' },
            evidence_terms: { type: 'array', items: { type: 'string' }, description: 'คำที่ควรพบใน Resume เพื่อยืนยันเงื่อนไข' },
          },
          required: ['field', 'value', 'evidence_terms'],
        },
      },
      reason: { type: 'string', description: 'เหตุผลสั้น ๆ ว่าทำไมเนื้องานนี้ควรค้นด้วยตำแหน่งเหล่านี้' },
    },
    required: ['family', 'family_label', 'positions', 'job_dna', 'hard_filters', 'reason'],
  },
};
...
export async function positionsFromDescription({ description, province, platform } = {}) {
  ...
  const res = await callModel({
    system:
      ...
      '\n\n## Hard Filter\nดึงเฉพาะเงื่อนไขบังคับที่ผู้ใช้เขียนชัดในเนื้องาน เช่น ต้องมีใบขับขี่หรือทักษะเฉพาะ ห้ามสร้างเงื่อนไขเพิ่มเอง ถ้าไม่มีให้ส่ง hard_filters เป็น [] แต่ยังต้องสรุป job_dna ของสายงาน\n\n## รูปแบบคำตอบ (บังคับ)\nตอบเป็น JSON object เดียว ใช้ key เป๊ะ ๆ ตามนี้ ห้ามใช้ job_family หรือ reasoning:\n{"family":"D","family_label":"📋 Service-Operational","positions":["พนักงานคลังสินค้า","พนักงานสโตร์","พนักงานแพ็คสินค้า","พนักงานขับโฟล์คลิฟท์","ธุรการคลัง","เจ้าหน้าที่จัดส่ง"],"job_dna":"ความถูกต้องและการทำงานตามระบบคลัง","hard_filters":[{"field":"license","value":"ใบขับขี่โฟล์คลิฟท์","evidence_terms":["ใบขับขี่โฟล์คลิฟท์","ใบอนุญาตขับโฟล์คลิฟท์"]}],"reason":"..."}',
    tool: DESC_TOOL,
  ...
  return {
    ...
    familyLabel: String(out.family_label || out.family || ''),
    positions,
    jobDna: String(out.job_dna || '').trim(),
    hardFilters: Array.isArray(out.hard_filters)
      ? out.hard_filters.map((f) => ({
          field: String(f?.field || 'other'),
          value: String(f?.value || '').trim(),
          evidence_terms: cleanList(f?.evidence_terms),
        })).filter((f) => f.value)
      : [],
    reason: String(out.reason || '').trim(),
    model: res.modelUsed,
