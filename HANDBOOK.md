...
6. ค้นหา resume IDs — **"ค้นกว้าง กรองแม่น"**: ส่งเฉพาะตำแหน่ง/คำค้นไปเว็บ ส่วน อายุ/วุฒิ/จังหวัด/เพศ ถูกย้ายไปกรองในระบบ (`src/core/candidate-match.js`) เพราะฟิลเตอร์ AND ซ้อนที่เว็บทำให้เหลือ 0 คน; ปิดพฤติกรรมนี้ได้ด้วย `STRICT_SITE_FILTERS=1`
7. ดึงรายละเอียดทีละคนตาม random delay
8. parse ข้อมูล → **เช็คเงื่อนไข local ก่อน reveal contact** (คนไม่ตรง = ข้าม ไม่เปลือง quota ไม่นับเข้า target; ข้อมูลขาด = ผ่านแบบ lenient), reveal contact ถ้า provider รองรับ, ดาวน์โหลด assets
