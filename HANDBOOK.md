...
|---|---|---|
| **ตำแหน่ง** | root: `api-scraper/` | nested: `api-scraper/api-scraper/` |
...
> 💡 **ถ้าจะพัฒนาต่อ ให้โฟกัสที่ Project B** Project A เก็บไว้เป็น reference ของ selector และ flow
...
```
api-scraper/                          ← Project A (demo-scaping)
├── scrape.js                         # main entry (npm run scrape)
├── auth.js                           # login มือ → เซฟ storageState (.auth/jobbkk.json)
├── download.js                       # re-download รูป/ไฟล์แนบจาก candidates.jsonl เดิม
├── config-popup.js                   # HTML popup เก็บ search criteria
├── jobbkk-filters.js                 # applyJobBkkFilters + กดค้นหา
├── resume-premium-search.js          # helper หน้า premium (popover/autocomplete)
├── scrape-timing.js                  # preflight + รอ login/หน้า resume
├── candidate-assets.js               # ดาวน์โหลดรูป/ไฟล์แนบ (4-strategy fallback)
├── core/
│   ├── env.js                        # envInt/envBool/sleep + timing jitter
│   ├── platform-resolve.js           # เลือก platform (jobbkk/jobthai/both)
│   ├── scrape-pipeline.js            # ★ orchestrator (runTalentScrape)
│   ├── candidate-dedupe.js           # defaultDedupeKey
│   └── candidate-export.js           # writeJsonl/writeCsv/buildReadableMarkdown
├── providers/
│   ├── registry.js                   # resolveProvider() — 4 platforms
│   ├── provider-contract.js          # JSDoc typedef (สัญญา interface)
│   ├── stub-provider.js              # stub ที่ throw "ยังไม่ implement"
│   ├── jobbkk/   (index + scrape-impl)   ← ของจริง
│   ├── jobthai/  (index + scrape-impl + jobthai-filters)  ← ของจริง
│   ├── jobdb/    (stub)
│   └── facebook/ (stub)
├── scripts/                          # explore-jobthai*.js, test-*.js (R&D)
├── output/                           # ผลลัพธ์ (gitignored)
│
└── api-scraper/                      ← Project B (api-scraper + web)
    ├── src/
...
ขับโดย `runTalentScrape()` ที่ [core/scrape-pipeline.js:328](api-scraper/core/scrape-pipeline.js):
...
browser จะถูกเรียกใหม่ก็ต่อเมื่อ session ตายจริง ๆ (logic `needsRelogin` / takeover ใน [pipeline.js](api-scraper/api-scraper/src/pipeline.js)) กลยุทธ์ CAPTCHA คือ "login ให้น้อยจน CAPTCHA แทบไม่โผล่" ([captcha.js](api-scraper/api-scraper/src/captcha.js))
...
- ทุก connection ตั้ง `search_path="<schema>",public` ([db/pool.js:9](api-scraper/api-scraper/src/db/pool.js))
...
[repositories.js](api-scraper/api-scraper/src/db/repositories.js) `upsertCandidate()`:
...
หัวใจอยู่ที่ [`runConnector(connector, criteria, runtime, opts)`](api-scraper/api-scraper/src/pipeline.js) (ชื่อ export คือ `runConnector` ไม่ใช่ `run`):
...
| Worker | ไฟล์ | บทบาท |
|---|---|---|
| **worker** | [worker.js](api-scraper/api-scraper/src/worker.js) | batch ง่าย ๆ: list connector ที่ enabled (ข้ามตัวที่ยัง cooldown) แล้ว `runConnector` ทีละตัวด้วย criteria จาก `.env` ไม่มี queue |
| **tasks-worker** | [tasks-worker.js](api-scraper/api-scraper/src/tasks-worker.js) | queue/scheduler: ดึง `dueTasks()` แล้วรันเป็น 3 เฟส **scraping → ocr → enrich** พร้อมรายงาน progress |
| **extract-worker** | [extract-worker.js](api-scraper/api-scraper/src/extract-worker.js) | OCR แยก process: ดึง asset ที่ `extract_status='pending'` 1 batch (`EXTRACT_BATCH`=20) ส่งเข้า Ollama แยกไว้เพื่อไม่ให้ OCR ถ่วง scrape |
...
2. **register** ใน [src/connectors/registry.js](api-scraper/api-scraper/src/connectors/registry.js) — เพิ่ม key ใน `PROVIDERS` map (จุดเดียว) ไม่ต้องแก้ pipeline

---

## 9. Anti-ban, Crypto, OCR

### 9.1 Anti-ban — [core/anti-ban.js](api-scraper/api-scraper/src/core/anti-ban.js)
...
### 9.2 Crypto — [db/crypto.js](api-scraper/api-scraper/src/db/crypto.js)
...
### 9.3 OCR / Ollama — [core/ollama.js](api-scraper/api-scraper/src/core/ollama.js)
...
### 10.1 Control API — [api/server.js](api-scraper/api-scraper/src/api/server.js)
...
### 10.2 CLI — [cli/connector.js](api-scraper/api-scraper/src/cli/connector.js)
...
- [lib/auth.ts](api-scraper/api-scraper/web/lib/auth.ts): `AzureADProvider` (`AZURE_AD_CLIENT_ID/SECRET/TENANT_ID`, scope `openid profile email User.Read`)
...
- [lib/db.ts](api-scraper/api-scraper/web/lib/db.ts): `pg.Pool` (cached, max 5) ตั้ง `search_path="${DB_SCHEMA}",public` → ชี้ schema เดียวกับ backend (`server-only`)
- [lib/repo.ts](api-scraper/api-scraper/web/lib/repo.ts): SQL ทั้งหมดต่อตารางร่วม (candidates/sources/assets/connectors/provider_limits/scrape_tasks/scrape_runs)
- [lib/actions.ts](api-scraper/api-scraper/web/lib/actions.ts): server actions (`'use server'`) ทุกตัวเรียก `requireSession()` ก่อน
...
- [api/assets/[id]/route.ts](api-scraper/api-scraper/web/app/api/assets/[id]/route.ts): stream bytea (auth-gated, validate UUID, cache `private, max-age=300`)
...
```bash
cd api-scraper/api-scraper/web
npm install
...
- Project B orchestration: [src/pipeline.js](api-scraper/api-scraper/src/pipeline.js)
- DB layer: [src/db/repositories.js](api-scraper/api-scraper/src/db/repositories.js), [src/db/schema.sql](api-scraper/api-scraper/src/db/schema.sql)
- Provider ตัวอย่าง: [src/providers/jobbkk/](api-scraper/api-scraper/src/providers/jobbkk/)
- Web data layer: [web/lib/repo.ts](api-scraper/api-scraper/web/lib/repo.ts), [web/lib/actions.ts](api-scraper/api-scraper/web/lib/actions.ts)
- Project A orchestration: [core/scrape-pipeline.js](api-scraper/core/scrape-pipeline.js)
